...
from utility.utils3d import shave3D
from utility.utils3d import pad3D
from utility.SegSRGAN import SegSRGAN
from utility.ImageReader import NIFTIReader
from utility.ImageReader import DICOMReader
from utility.normalization import Normalization
from keras.engine import saving
import 2D_utils.interpolation as inter
...
class SegSRGAN_test(object):
    ...
    def __init__(self, weights, patch1, patch2, is_conditional, u_net_gen,is_residual, first_generator_kernel,
        ...
        self.res_tensor = np.expand_dims(np.expand_dims(np.ones([patch1, patch2]) * self.resolution, axis=0),
                                        axis=0)
        ...
        if self.is_conditional is True:
            if not by_batch:
                ...
                bar = progressbar.ProgressBar(maxval=len(np.arange(0, height - self.patch1 + 1, step)) * len(
                    np.arange(0, width - self.patch2 + 1, step))).start()
                print('Patch=', self.patch1)
                print('Step=', step)
                for idx in range(0, height - self.patch1 + 1, step):
                    for idy in range(0, width - self.patch2 + 1, step):
                            # Cropping image
                            test_patch = test_image[idx:idx + self.patch1, idy:idy + self.patch2]
                            image_tensor = test_patch.reshape(1, 1, self.patch1, self.patch2).\
                                astype(np.float32)
                            predict_patch = self.generator.predict([image_tensor, self.res_tensor], batch_size=1)

                            # Adding
                            temp_hr_image[idx:idx + self.patch1, idy:idy + self.patch2] += predict_patch[0, 0, :, :]
                            temp_seg[idx:idx + self.patch1, idy:idy + self.patch2] += predict_patch[0, 1, :, :]
                            weighted_image[idx:idx + self.patch1, idy:idy + self.patch2] += np.ones_like(predict_patch[0, 0,:, :])
                ...
                patches = np.array([[test_image[idx:idx + patch1, idy:idy + patch2]] for idx in
                                    range(0, height - patch1 + 1, step) for idy in range(0, width - patch2 + 1, step)])

                indice_patch = np.array([(idx, idy) for idx in range(0, height - patch1 + 1, step) for idy in
                                         range(0, width - patch2 + 1, step)])
                ...
                for i in range(indice_patch.shape[0]):
                    ...
                    indice_patch[i][1]:indice_patch[i][1] + patch2] += pred[i, 0,:, :]
                    temp_seg[indice_patch[i][0]:indice_patch[i][0] + patch1, indice_patch[i][1]:indice_patch[i][1] + patch2] += pred[i, 1, :, :]
                    weight[indice_patch[i][0]:indice_patch[i][0] + patch1, indice_patch[i][1]:indice_patch[i][1] + patch2] + np.ones_like(
            ...
            if not by_batch:
                ...
                for idx in range(0, height - self.patch1 + 1, step):
                    for idy in range(0, width - self.patch2 + 1, step):
                            # Cropping image
                            test_patch = test_image[idx:idx + self.patch1, idy:idy + self.patch2]
                            image_tensor = test_patch.reshape(1, 1, self.patch1, self.patch2).astype(
                                np.float32)
                ...
                for i in range(indice_patch.shape[0]):
                    ...
                    indice_patch[i][1]:indice_patch[i][1] + patch2] += pred[i, 1, :, :]
                    weight[indice_patch[i][0]:indice_patch[i][0] + patch1,
                    indice_patch[i][1]:indice_patch[i][1] + patch2] + np.ones_like(
                        weight[indice_patch[i][0]:indice_patch[i][0] + patch1,
                        indice_patch[i][1]:indice_patch[i][1] + patch2])
...
def segmentation(input_file_path, step, new_resolution, path_output_cortex, path_output_hr, weights_path,
    ...
    if patch is not None:
        ...
        border = (int(interpolated_image.shape[0] % 4), int(interpolated_image.shape[1] % 4))
        border_to_add = (4 - border[0], 4 - border[1])
    ...
    if ((step>patch1) |  (step>patch2)) & (patch is not None) :

        raise AssertionError('The step need to be smaller than the patch size')

    if (np.shape(padded_interpolated_image)[0]<patch1)|(np.shape(padded_interpolated_image)[1]<patch2):

        raise AssertionError('The patch size need to be smaller than the interpolated image size')
    print("Just before the test")
    # Loading weights
    segsrgan_test_instance = SegSRGAN_test(weights_path, patch1, patch2, is_conditional, u_net_gen,is_residual,
